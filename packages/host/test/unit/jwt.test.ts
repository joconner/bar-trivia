...
// Build a test JWT with the given payload. Uses Buffer for reliable base64
// encoding; converts to base64url and preserves padding so atob can decode it.
...
describe('decodeToken', () => {
  it('returns the parsed payload for a valid token', () => {
    const token = makeToken({ sub: 'u1', role: 'host' })
    expect(decodeToken(token)).toMatchObject({ sub: 'u1', role: 'host' })
  })
