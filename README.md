...
`render.yaml` at the repo root defines a single free-tier Web Service. The service builds all three React clients and serves them as static files from the NestJS server. Uses SQLite so **no external database is required** — zero credentials needed beyond a Render account.
...
1. Click the **Deploy to Render** button above (or go to Render → New → Web Service → connect `joconner/bar-trivia`). Render auto-detects `render.yaml`.
2. `JWT_SECRET` and `COOKIE_SECRET` are auto-generated. No other env vars are required for a demo.
3. Hit **Create Web Service**. The first deploy builds the three React apps and runs `prisma db push && prisma db seed` before starting the server (~4 min).
