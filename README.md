...
[![Deploy to Render](https://render.com/images/deploy-to-render-button.svg)](https://render.com/deploy?repo=https://github.com/joconner/bar-trivia)

**Prerequisites:**
- A free [Neon](https://neon.tech) PostgreSQL project (no credit card required). Copy the connection string after creating the project — it looks like `postgresql://user:pass@ep-xxx.us-east-2.aws.neon.tech/neondb?sslmode=require`.
- A [Render](https://render.com) account (free, sign in with GitHub).

**One-time setup:**

1. Create a free Neon project at [neon.tech](https://neon.tech). Copy the connection string from the dashboard.
2. Click the **Deploy to Render** button above (or go to Render → New → Web Service → connect `joconner/bar-trivia`). Render auto-detects `render.yaml`.
3. In the Render environment variables panel, set **`DATABASE_URL`** to your Neon connection string. `JWT_SECRET` and `COOKIE_SECRET` are auto-generated by Render.
4. Hit **Create Web Service**. The first deploy builds the three React apps and runs `prisma db push && prisma db seed` before starting the server (~4 min).
