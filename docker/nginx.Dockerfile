...
# TV build — VITE_SERVER_URL="" → same-origin socket.io via nginx proxy on 80.
# The QR code's player URL is derived at runtime from window.location.origin
# (see packages/tv/src/views/LobbyView.tsx), so no build-time URL is baked in.
# vite.config.ts pins base: '/tv/' so assets resolve correctly under the prefix.
FROM base AS tv-build
RUN cd packages/tv && VITE_SERVER_URL="" npx vite build

# Player build — already uses io("/") for same-origin; vite.config.ts pins
# base: '/player/' for asset paths under the prefix.
...
# vite.config.ts pins base: '/host/' and PWA manifest paths to match.
